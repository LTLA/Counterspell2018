...
Furthermore, the squeezing effect interferes with gene-level interpretation of the denosied expression values. 
Log-fold changes are shrunk towards zero for the subset of genes responsible for the secondary separation (Figure~\ref{fig:fourclusters}c, d).
...
(c) Log-fold changes with respect to the primary or secondary separations, computed from the denoised expression values.
Each point represents a gene and is coloured according to whether it is DE (blue) or not (grey).
(d) Same as (c) but using the original (log-)counts.}
